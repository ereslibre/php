...
EXEEXT =
CC = /php/wasi-sdk-16.0/bin/clang --sysroot=/php/wasi-sysroot -D_WASI_EMULATED_MMAN=1 -D_WASI_EMULATED_SIGNAL=1 -D_WASI_EMULATED_PROCESS_CLOCKS=1 -D_WASI_EMULATED_GETPID=1 -D_POSIX_SOURCE=1 -D_GNU_SOURCE=1 -DHAVE_FORK=0 -DSQLITE_OMIT_LOAD_EXTENSION=1 -D__wasi__=1 -DSQLITE_THREADSAFE=0 -DSQLITE_OMIT_WAL=1 -DSQLITE_DEFAULT_SYNCHRONOUS=0 -DSQLITE_PAGER_SYNCHRONOUS=1 -DSQLITE_OMIT_RANDOMNESS
...
.PHONY: build-shell
build-shell:
	podman run -d -v ${PWD}:/root/php-src -it ghcr.io/ereslibre/php:1005 sleep infinity

.PHONY: run
run:
	podman rm -f mod_wasm; podman run -d --name mod_wasm -p8080:8080 -v ${PWD}/sapi/cgi:/usr/local/apache2/wasm_modules -v ${HOME}/Downloads/wp-sqlite-db:/usr/local/apache2/htdocs localhost/httpd-mod-wasm:latest; clear; podman logs -f mod_wasm
